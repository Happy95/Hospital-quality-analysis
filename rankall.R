rankall <- function(outcome, num = "best"){
  ...
  state_arr <- sort(as.character(unique(OC$State)))
  Info <- data.frame(hosp = character(0), region = character(0))
  for(i in 1:length(state_arr)){
    Hname <- rankhospital(state_arr[i], outcome, num)
    dataT <- data.frame(hosp = Hname, region = state_arr[i])
    Info <- rbind(Info, dataT)
  }
  names(Info) <- c("Hospital", "States")
  Info
}